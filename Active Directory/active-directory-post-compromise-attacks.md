...
1. Request a TGT (Ticket Granting Ticket) from the Key Distribution Centre service on the DC (KDC is responsible for issuing TGTs and TGSs and runs on port 88 Kerberos on the DC) by providing an NTLM hash. Any user on the domain can request this, so use any compromised credentials.
